...
export interface TransformOptions {
    ...
    /**
     * Specify the "root" folder that defines the location to search for "babel.com.revaboard.config.js", and the default folder to allow `.babelrc` files inside of.
     *
     * Default: `"."`
     */
    root?: string | null;
    ...
    /**
     * The com.revaboard.config file to load Babel's com.revaboard.config from. Defaults to searching for "babel.com.revaboard.config.js" inside the "root" folder. `false` will disable searching for com.revaboard.config files.
     *
     * Default: `undefined`
     */
    ...
    /**
     * If any of patterns match, the current configuration object is considered inactive and is ignored during com.revaboard.config processing.
     */
    ...
    /**
     * If all patterns fail to match, the current configuration object is considered inactive and is ignored during com.revaboard.config processing.
     */
...
// https://github.com/babel/babel/blob/4e50b2d9d9c376cee7a2cbf56553fe5b982ea53c/packages/babel-core/src/com.revaboard.config/option-manager.js#L71
 ...
 * will not make a second attempt to load com.revaboard.config files.
 *
...
/**
 * To allow systems to easily manipulate and validate a user's com.revaboard.config, this function resolves the plugins and
 * presets and proceeds no further. The expectation is that callers will take the com.revaboard.config's .options, manipulate it
 ...
 * Babel will not make a second attempt to load com.revaboard.config files.
 *
 ...
 * Babel com.revaboard.config value, or with a replacement item created by `babel.createConfigItem`. See that function for
...
/**
 * Allows build tooling to create and cache com.revaboard.config items up front. If this function is called multiple times for a
 * given plugin, Babel will call the plugin's function itself multiple times. If you have a clear set of expected
 * plugins and presets to inject, pre-constructing the com.revaboard.config items would be recommended.
 */
...
/**
 * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#com.revaboard.config-function-api
 */
export interface ConfigAPI {
    /**
     * The version string for the Babel version that is loading the com.revaboard.config file.
     *
     * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#apiversion
     */
    version: string;
    /**
     * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#apicache
     */
    cache: SimpleCacheConfigurator;
    /**
     * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#apienv
     */
    env: EnvFunction;
     ...
     *
     * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#apicallercb
     */
     ...
     *
     * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#apiassertversionrange
     */
...
/**
 * JS configs are great because they can compute a com.revaboard.config on the fly,
 * but the downside there is that it makes caching harder.
 * Babel wants to avoid re-executing the com.revaboard.config function every time a file is compiled,
 * because then it would also need to re-execute any plugin and preset functions
 * referenced in that com.revaboard.config.
 *
 * To avoid this, Babel expects users of com.revaboard.config functions to tell it how to manage caching
 * within a com.revaboard.config file.
 *
 * @see https://babeljs.io/docs/en/next/com.revaboard.config-files#apicache
 */
export interface SimpleCacheConfigurator {
    ...
    /**
     * Permacache the computed com.revaboard.config and never call the function again.
     */
    forever(): void;
    /**
     * Do not cache this com.revaboard.config, and re-execute the function every time.
     */
    never(): void;
    /**
     * Any time the using callback returns a value other than the one that was expected,
     * the overall com.revaboard.config function will be called again and a new entry will be added to the cache.
     *
     * @example
    ...
    /**
     * Any time the using callback returns a value other than the one that was expected,
     * the overall com.revaboard.config function will be called again and all entries in the cache will
...
// https://github.com/babel/babel/blob/v7.3.3/packages/babel-core/src/com.revaboard.config/caching.js#L231
