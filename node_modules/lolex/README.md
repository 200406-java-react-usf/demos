...
configurable interval `com.revaboard.config.advanceTimeDelta` (default: 20ms). Meaning time would
...
### `var clock = lolex.install([com.revaboard.config])`
Installs lolex using the specified com.revaboard.config (otherwise with epoch `0` on the global scope). The following configuration options are available

Parameter | Type | Default | Description
--------- | ---- | ------- | ------------
`com.revaboard.config.target`| Object | global | installs lolex onto the specified target context
`com.revaboard.config.now` | Number/Date | 0 | installs lolex with the specified unix epoch
`com.revaboard.config.toFake` | String[] | ["setTimeout", "clearTimeout", "setImmediate", "clearImmediate","setInterval", "clearInterval", "Date", "requestAnimationFrame", "cancelAnimationFrame", "requestIdleCallback", "cancelIdleCallback", "hrtime"] | an array with explicit function names to hijack. *When not set, lolex will automatically fake all methods **except** `nextTick`* e.g., `lolex.install({ toFake: ["setTimeout","nextTick"]})` will fake only `setTimeout` and `nextTick`
`com.revaboard.config.loopLimit` | Number | 1000 | the maximum number of timers that will be run when calling runAll()
`com.revaboard.config.shouldAdvanceTime` | Boolean | false | tells lolex to increment mocked time automatically based on the real system time shift (e.g. the mocked time will be incremented by 20ms for every 20ms change in the real system time)
`com.revaboard.config.advanceTimeDelta` | Number | 20 | relevant only when using with `shouldAdvanceTime: true`. increment mocked time by `advanceTimeDelta` ms every `advanceTimeDelta` ms change in the real system time.
...
Removes all timers and ticks without firing them, and sets `now` to `com.revaboard.config.now`
that was provided to `lolex.install` or to `0` if `com.revaboard.config.now` was not provided.
