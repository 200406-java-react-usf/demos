...
validate((com.revaboard.config: Object), (options: ValidationOptions)); // => {hasDeprecationWarnings: boolean, isValid: boolean}
```
...
- `deprecatedConfig` – optional object with deprecated com.revaboard.config keys.
- `exampleConfig` – the only **required** option with configuration against which you'd like to test.
- `recursive` - optional boolean determining whether recursively compare `exampleConfig` to `com.revaboard.config` (default: `true`).
...
```js
validate(com.revaboard.config, {exampleConfig});
```

Example with slight modifications:

```js
validate(com.revaboard.config, {
